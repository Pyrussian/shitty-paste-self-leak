...
void CGUI::Update()
{
	...
	for (auto window : Windows)
	{
		if (window->m_bIsOpen)
		{
			...
			Render::Text((centerW * 2) - 50, 25, Color(0, 120, 250, (MenuAlpha - 1)), Render::Fonts::ESP, "Project Cocaine");
			Render::Text((centerW * 2) - 50,25, Color(255, 10, 165, MenuAlpha), Render::Fonts::ESP, "Cocaine");
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	tinyxml2::XMLElement *Root = Doc.NewElement("Project Cocaine");
	...
	{
		MessageBox(NULL, "Unable to save config file. Please try again", "Cocaine", MB_OK);
	}
